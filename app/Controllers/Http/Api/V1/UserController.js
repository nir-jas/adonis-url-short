...
class UserController {
	async getUsers({ request,response}) {
		...
		try {
			...
			if(params.sort && params.sort=='name')
				users = users.orderBy('name',params.order)
				else if(params.sort && params.sort=='email')
					users = users.orderBy('email',params.order)
				else if(params.sort && params.sort=='created_at')
					users = users.orderBy('created_at',params.order)
				else if(params.sort && params.sort=='updated_at')
					users = users.orderBy('updated_at',params.order)
	...
	async changePassword({ request, view, response, auth }){
		...
		try {
			...
			await User.query().where('id',params.user_id).update({'password':await Hash.make(params.new_password)})
